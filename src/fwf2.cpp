...
/*New in this version:
    -Separated R, G, and B color scaling
*/
...
float rscale,tscale,res,itmax,tstart=0,tend,fov,rotx=0,roty=0,rotz=0,locx=0,locy=0,locz=0,xaxis[3],dp[3],tempaxis[3],linewidth,rdensity,gdensity,bdensity,zsdensity,stretch;
int dispnormals;
		...
		case 1:
			...
			glNewList(1,GL_COMPILE);
				...
				glBegin(GL_LINE_STRIP);
					for(t=tend;t>tstart;t-=1.0/pow(2.0,res))
					{
						ctemp[0]=(cos(rdensity*t)+1)/2;
                        ctemp[1]=(sin(gdensity*t)+1)/2;
                        ctemp[2]=(sin(bdensity*t)+1)/2;
						yaxis[0]=0; yaxis[1]=1; yaxis[2]=0;//yaxis points from foo central axis to foo
						zaxis[0]=0; zaxis[1]=0; zaxis[2]=1;//zaxis is the central axis of foo
						foov(t,p); p[0]=0; p[1]=0;
...
void init(void)
{
	...
	gui1data.boxes.back().setfunc(empty);
	gui1data.newbox(127,282,20,70,20,.1,0,1000,1,10,"Red Density","",&rdensity);
	gui1data.boxes.back().setfunc(empty);
	gui1data.newbox(127,282+27,20,70,20,.1,0,1000,1,1,"Green Density","",&gdensity);
	gui1data.boxes.back().setfunc(empty);
	gui1data.newbox(127,282+27*2,20,70,20,.1,0,1000,1,0,"Blue Density","",&bdensity);
	gui1data.boxes.back().setfunc(empty);
	gui1data.newbox(127,282+27*3,20,70,20,.05,0,100,1,.1,"Z Shade Dens.","",&zsdensity);
	gui1data.boxes.back().setfunc(empty);
